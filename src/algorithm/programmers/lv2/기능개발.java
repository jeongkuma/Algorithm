 ...
 * 두 번째 기능은 30%가 완료되어 있고 하루에 30%씩 작업이 가능하므로 3일간 작업 후 배포가 가능합니다. 하지만 이전 첫 번째 기능이 아직 완성된 상태가 아니기 때문에 첫 번째 기능이 배포되는 7일째
 * 배포됩니다.
 ...
 *
 * 입출력 예 #2
 * 모든 기능이 하루에 1%씩 작업이 가능하므로, 작업이 끝나기까지 남은 일수는 각각 5일, 10일, 1일, 1일, 20일, 1일입니다. 어떤 기능이 먼저 완성되었더라도 앞에 있는 모든 기능이 완성되지 않으면
 * 배포가 불가능합니다.
 *
...
//        int[] speeds = new int[]{1, 1, 1, 1, 1, 1};
        ...
        int[] progresses = new int[]{20, 99, 93, 30, 55, 10};
...
//        map.forEach((key, value) -> System.out.println(value));
        ...
        for (int s = 0; s < setList.size(); s++) {
            System.out.println(setList.get(s));
        }
